 ...
 * @最后修改内容:
 */
public class QSGifFragment extends BaseFragment implements QsImageView, PullToRefreshLayout
        .OnRefreshListener {
    ...
    public static QSGifFragment getInstence(int type) {
        Bundle bundle = new Bundle();
        bundle.putInt("type", type);
        QSGifFragment fragment = new QSGifFragment();
    ...
    protected void initViews() {
        ...
        mApapter = new QSImageApapter(getActivity(), "3");
        mList.setAdapter(mApapter);

        QSImageBean bean = new QSImageBean();
        bean = (QSImageBean) MyApplication.ACACHE.getAsObject("QSGIFBean");
    ...
    public void showDatas(QSImageBean bean) {
        ...
        if (SAVE){
            MyApplication.ACACHE.remove("QSGIFBean");
            MyApplication.ACACHE.put("QSGIFBean", bean);
            SAVE = false;
        }
    }
    ...
    public void onRefresh(PullToRefreshLayout pullToRefreshLayout) {
        pageToken = 1;
        mPresenter.setDatas(getActivity(), "3", pageToken + "");
    }
