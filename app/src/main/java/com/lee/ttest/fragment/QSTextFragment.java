...
import com.lee.ttest.adapter.QSTextApapter;
 ...
 * @最后修改内容:
 */
public class QSTextFragment extends BaseFragment implements QsTextView, PullToRefreshLayout
        .OnRefreshListener {
    ...
    private QSTextListPresenter mPresenter;
    private int lastVisibleItemPosition = 0;// 标记上次滑动位置  
    private boolean scrollFlag = false;
    private List<QSTextBean.ShowapiResBodyBean.ContentlistBean> mDataBeen;
    private QSTextApapter mApapter;

    public static QSTextFragment getInstence(int type) {
        Bundle bundle = new Bundle();
        bundle.putInt("type", type);
        QSTextFragment fragment = new QSTextFragment();
    ...
    protected void initViews() {
        ...
        mApapter = new QSTextApapter(getActivity());
        mList.setAdapter(mApapter);

        QSTextBean bean = new QSTextBean();
        bean = (QSTextBean) MyApplication.ACACHE.getAsObject("QSTextBean");
        ...
        mPresenter = new QSTextListPresenterImpl(this);
    }
    ...
    @Override
    public void showDatas(QSTextBean bean) {
        ...
        if (SAVE){
            MyApplication.ACACHE.remove("QSTextBean");
            MyApplication.ACACHE.put("QSTextBean", bean);
            SAVE = false;
        }
    }
