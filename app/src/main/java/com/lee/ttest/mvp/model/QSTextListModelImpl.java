 ...
 * @最后修改内容:
 */
public class QSTextListModelImpl implements QsTextModel {

    @Override
    public void loadDatas(Context context, String pageToken, final Callback callback) {
            if (!"".equals(pageToken)) {
                OkHttpUtils.get().url(Apis.URL_QSTEXT)
                        ...
                        .execute(new StringCallback() {
                            ...
                            public void onResponse(String response, int id) {
                                try {
                                    QSTextBean bean = JSONObject.parseObject(response, QSTextBean.class);
